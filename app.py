...
def login():
    ...
    with st.form("login_form"):
        username_options = list(USERS_DB.keys())
        ...
        if submitted:
            if USERS_DB[username]["password"] == password:
                ...
                st.session_state.full_name = USERS_DB[username]["full_name"]
...
def show_sidebar():
    ...
    full_menu = ["Dashboard"] + upload_menu + external_menu + task_menu + mdr_menu + ["Document History", "Review Queue"]
...
def main_app():
    ...
    # -------------------------
    # DASHBOARD (FILTERED BY ROLE + PERSONAL TASKS)
    # -------------------------
    if menu == "Dashboard":
        st.title("Dashboard")
    ...
    # -------------------------
    # IMPORT MDR FROM EXCEL (Admin/DCC only)
    # -------------------------
    ...
    # -------------------------
    # UPLOAD DOCUMENT (Responsible or Admin)
    # -------------------------
    ...
    # -------------------------
    # EXTERNAL INTAKE (for emails from Architect/Client)
    # -------------------------
        ...
        with st.form("external_form"):
            ...
            email_reference = st.text_area("Email Reference (subject, date, sender, etc.)", placeholder="e.g., Email from Architect on 2025-05-14: Structural comments")
            ...
            if submit:
                if not uploaded_file:
                    ...
                    if not doc_no:
                        ...
                        existing = st.session_state.mdr[st.session_state.mdr["Doc No"].str.startswith(prefix)]
                        count = len(existing) + 1
                        doc_no = f"{prefix}-{count:04d}"
                    if doc_no in st.session_state.mdr["Doc No"].values:
                        st.error("Document Number already exists. Please use a different one.")
                    else:
                        try:
                        ...
                        save_data()
                        st.success(f"External document from {source} registered successfully as {doc_no} and sent to DCC review.")

    # -------------------------
    # ASSIGN TASK (Admin, DCC, Lead)
    # -------------------------
        ...
        with st.form("task_form"):
            # انتخاب مدرک مرتبط (اختیاری)
            doc_list = [""] + st.session_state.mdr["Doc No"].tolist()
            selected_doc = st.selectbox("Related Document (optional)", doc_list)
            task_desc = st.text_area("Task Description")
            # لیست کاربران (به جز خود شخص)
            user_list = [f"{usr} ({USERS_DB[usr]['full_name']} - {USERS_DB[usr]['role']})" for usr in USERS_DB.keys() if usr != st.session_state.username]
            selected_user_display = st.selectbox("Assign To", user_list)
            assigned_to_username = selected_user_display.split(" ")[0]  # استخراج نام کاربری
    ...
    # -------------------------
    # REVIEW QUEUE (with return to previous step)
    # -------------------------
        ...
        if len(role_queue) == 0:
            ...
            for i, row in role_queue.iterrows():
                with st.expander(f"{row['Doc No']} | Rev {row['Revision']} | Step: {row['Workflow Step']}"):
                    ...
                    if "Email Reference" in row and row["Email Reference"]:
                    ...
                    # فرم ثبت کامنت ساده (بدون تغییر وضعیت)
                    ...
                    if row["Status"] == "Rejected":
                        if st.button("📤 Revise & Resubmit", key=f"resubmit_{i}"):
                            if st.session_state.role not in ["Responsible", "Admin"]:
                                st.error("Only Responsible or Admin can resubmit a rejected document.")
                            else:
                                ...
                                save_data()
                                st.success("New revision created. Please upload a corrected document.")
                                st.rerun()
                        continue
                    ...
                    with col1:
                        if st.button("✅ Approve", key=f"approve_{i}"):
                            ...
                            if step_index < len(WORKFLOW) - 1:
                                ...
                                doc_no = row["Doc No"]
                                mdr_idx = st.session_state.mdr[st.session_state.mdr["Doc No"] == doc_no].index[0]
                                ...
                                if next_step == "Responsible" and step_index + 1 == len(WORKFLOW)-1:
                                    ...
                                    st.success("Document fully approved and workflow completed!")
                                else:
                                ...
                                mdr_idx = st.session_state.mdr[st.session_state.mdr["Doc No"] == doc_no].index[0]
                                st.session_state.mdr.loc[mdr_idx, "Status"] = "Approved"
                                st.success("Document approved and workflow finished.")
                            save_data()
                            st.rerun()
                    ...
                    with col2:
                        if st.button("❌ Reject (Return to previous step)", key=f"reject_{i}"):
                            ...
                            comment_text = st.text_area("Additional comment (optional)", key=f"reject_comment_{i}")
                            if not reason:
                                st.error("Please provide a reason for returning the document")
                            else:
                                if comment_text:
                                ...
                                if step_index > 0:
                                    ...
                                    st.warning(f"Document returned to {prev_step} step. Reason: {reason}")
                                else:
                                    st.warning("Document is already at first step. Cannot return further.")
                                save_data()
                                st.rerun()
                    ...
                    with col3:
                        ...
                        if st.session_state.get(f"show_return_form_{i}", False):
                            with st.form(key=f"return_form_{i}"):
                                ...
                                return_file = st.file_uploader("Attach file (e.g., marked-up PDF)", type=["pdf", "docx", "txt", "jpg", "png"], key=f"return_file_{i}")
                                submit_return = st.form_submit_button("Submit Return Request")
                                if submit_return:
                                    if not return_reason:
                                        st.error("Please provide a reason for requesting changes")
                                    else:
                                        ...
                                        if return_file is not None:
                                            ...
                                            add_comment(i, f"Attached file for change request: {return_file.name}")
                                        current_step = row["Workflow Step"]
                                        step_index = WORKFLOW.index(current_step)
                                        if step_index > 0:
                                            prev_step = WORKFLOW[step_index - 1]
                                            ...
                                            st.warning(f"Document returned to {prev_step} for changes.")
                                        else:
                                            st.warning("Document already at first step.")
                                        save_data()
    ...
    # -------------------------
        ...
        if len(st.session_state.mdr) == 0:
            ...
            if len(doc_list) == 0:
                st.info("No document numbers found.")
            else:
                selected_doc = st.selectbox("Select Document", doc_list)
                doc_history = st.session_state.documents[st.session_state.documents["Doc No"] == selected_doc]
                if "Revision" in doc_history.columns:
                    doc_history = doc_history.sort_values("Revision")
                if len(doc_history) == 0:
                    st.info("No uploads for this document yet.")
                else:
                    for _, rev_row in doc_history.iterrows():
                        with st.expander(f"Revision {rev_row['Revision']} - {rev_row['Date']} - Status: {rev_row['Status']}"):
                            ...
                            if "Source" in rev_row and rev_row["Source"]:
                                st.info(f"**Source:** {rev_row['Source']}")
                            if "Email Reference" in rev_row and rev_row["Email Reference"]:
